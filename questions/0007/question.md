...
Design a parameterizable multiplexer that selects one of multiple input data streams based on a sel signal. The module should be configurable for different data widths and number of inputs.
...
- **Inputs**:
  ...
  - `sel[SELECT_WIDTH-1:0]`: Select signal
- **Output**:
...
| `SELECT_WIDTH` | 2 | Width of sel signal (determines number of inputs) |

### Functional Requirements

1. **Data Selection**: Select one input based on sel signal value
...
- `sel = 2'b00` → `data_out = input_0`
- `sel = 2'b01` → `data_out = input_1`
- `sel = 2'b10` → `data_out = input_2`
- `sel = 2'b11` → `data_out = input_3`
