...
architecture rtl of median_calc is
  ...
  constant SORT   : std_logic_vector(1 downto 0) := "10";
  ...
  begin
    if rising_edge(clk) then
      if rst_n = '0' then
        ...
        out_valid_s <= '0';
        out_median  <= (others => '0');
        count       <= 0; wr_idx <= 0; i_cnt <= 0; j_cnt <= 0;
        swapped     <= '0';
        k := 0;
        ...
        case state is
          ...
          when SORT =>
            if count <= 1 then
              state <= OUTPUT; out_valid_s <= '1'; out_median <= arr(0);
              ...
              if swapped = '0' or i_cnt >= count - 2 then
                state <= OUTPUT; out_valid_s <= '1'; out_median <= arr(count/2);
              else
                i_cnt   <= i_cnt + 1;
                j_cnt   <= 0;
                swapped <= '0';
