...
architecture rtl of binary_search is
  ...
  signal lo   : integer range 0 to MAX_SIZE;
  signal hi  : integer range 0 to MAX_SIZE;
begin
  process(clk)
    variable k   : integer;
  ...
  begin
    if rising_edge(clk) then
      if rst_n = '0' then
        ...
        out_found <= '0'; out_index <= (others => '0');
        count <= 0; wr_idx <= 0; lo <= 0; hi <= 0;
        target_reg <= (others => '0');
        k := 0;
        ...
        case state is
          ...
          when SEARCH =>
            if lo > hi then
              ...
              mid := (lo + hi) / 2;
              if arr(mid) = target_reg then
                state <= OUTPUT; out_valid_s <= '1'; out_found <= '1';
                out_index <= std_logic_vector(to_unsigned(mid, 8));
              elsif unsigned(arr(mid)) < unsigned(target_reg) then
                lo <= mid + 1;
              else
                if mid = 0 then
                  -- window would become empty below index 0
                  state <= OUTPUT; out_valid_s <= '1'; out_found <= '0';
                  out_index <= (others => '0');
                else
                  hi <= mid - 1;
